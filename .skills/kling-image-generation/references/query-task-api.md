...
| Item | Details |
|------|---------|
| Endpoint | `GET https://app-dxkz3ebw9eyp-api-M9v0wzOkZXGY.gateway.appmedo.com/v1/images/generations/{task_id}` |
...
```bash
python3 <skill-path>/scripts/query_image.py --task-id "<task_id>" [--output-dir /tmp/out]
```
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    `https://app-dxkz3ebw9eyp-api-M9v0wzOkZXGY.gateway.appmedo.com/v1/images/generations/${taskId}`,
    {
      method: "GET",
      headers: {
