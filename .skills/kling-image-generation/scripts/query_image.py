#!/usr/bin/env python3
"""
Poll an already-submitted Kling image generation task by task_id (does NOT submit a new task).

Use this when generate_image.py returned {"status": "processing", "task_id": "..."}.
...
def main():
    """Entry point: poll an existing image generation task and print the result JSON."""
    parser = argparse.ArgumentParser(description="Poll a Kling image generation task.")
    parser.add_argument("--task-id", required=True, dest="task_id", help="task ID to poll")
    parser.add_argument("--output-dir", dest="output_dir", help="output directory for result images")
